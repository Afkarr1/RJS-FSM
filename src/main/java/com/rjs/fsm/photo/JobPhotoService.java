...
public class JobPhotoService {
    ...
    public JobPhotoResponse uploadPhoto(UUID jobId, MultipartFile file, UUID uploaderId) {
        ...
        // Check permission: admin can always upload, technician only for their own jobs
        User uploader = userRepo.findByIdAndTenantId(uploaderId, tenantId)
                .orElseThrow(() -> new ForbiddenException("User tidak ditemukan"));

        if (uploader.getRole() != UserRole.ADMIN) {
    ...
    public JobPhoto getPhoto(UUID photoId) {
        UUID tenantId = TenantContext.require();
        JobPhoto photo = photoRepo.findById(photoId)
